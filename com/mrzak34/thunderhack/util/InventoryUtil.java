...
public class InventoryUtil implements Util
{
    ...
    public static int getXPAtHotbar() {
        for (int i = 0; i < 9; ++i) {
            final ItemStack itemStack = Util.mc.player.inventory.getStackInSlot(i);
    ...
    public static int getCappuchinoAtHotbar() {
        for (int i = 0; i < 9; ++i) {
            ...
            if (itemStack.getItem() == Items.POTIONITEM) {
                if (itemStack.getItem().getItemStackDisplayName(itemStack).equals("\u041d\u0435\u0441\u043e\u0437\u0434\u0430\u0432\u0430\u0435\u043c\u043e\u0435 \u0437\u0435\u043b\u044c\u0435")) {
                    return i;
                }
            }
        }
    ...
    public static int getCappuchinoAtInv() {
        for (int i = 36; i >= 0; --i) {
            final Item item = InventoryUtil.mc.player.inventory.getStackInSlot(i).getItem();
            final ItemStack itemStack = Util.mc.player.inventory.getStackInSlot(i);
            if (itemStack.getItem() == Items.POTIONITEM) {
        ...
        switch (mode) {
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    ...
                    switchedItemSwitched[0] = false;
                    Thunderhack.inventoryManager.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
