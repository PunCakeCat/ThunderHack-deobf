...
public class BowSpam extends Module
{
    ...
    public BowSpam() {
        super("BowSpam", "\u0421\u043f\u0430\u043c\u0438\u0442 \u0441\u0442\u0440\u0435\u043b\u0430\u043c\u0438", Category.COMBAT, true, false, false);
    ...
    public void onUpdateWalkingPlayer(final UpdateWalkingPlayerEvent event) {
        ...
        if (this.autoSwitch.getValue() && InventoryUtil.findHotbarBlock(ItemBow.class) != -1 && this.ownHealth.getValue() <= EntityUtil.getHealth((Entity)BowSpam.mc.player) && (!this.onlyWhenSave.getValue() || EntityUtil.isSafe((Entity)BowSpam.mc.player))) {
            ...
            if (target != null && (!(crystal = Thunderhack.moduleManager.getModuleByClass(NewAC.class)).isOn() || !InventoryUtil.holdingItem(ItemEndCrystal.class))) {
                ...
                Thunderhack.rotationManager.lookAtVec3d(xPos, yPos, zPos);
                if (BowSpam.mc.player.getHeldItemMainhand().getItem() instanceof ItemBow) {
                    this.switched = true;
                }
            }
        ...
        if (this.mode.getValue() == Mode.FAST && (this.offhand || BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) && BowSpam.mc.player.isHandActive()) {
            ...
            final float f3 = this.tpsSync.getValue() ? Thunderhack.serverManager.getTpsFactor() : 1.0f;
            if (f >= f2 * f3) {
    ...
    public void onUpdate() {
        ...
        switch (this.mode.getValue()) {
            case AUTORELEASE: {
                ...
                if (!this.timer.passedMs((int)(this.delay.getValue() * (this.tpsSync.getValue() ? Thunderhack.serverManager.getTpsFactor() : 1.0f)))) {
                    break;
                }
    ...
    private EntityPlayer getTarget() {
        ...
        for (final EntityPlayer player : BowSpam.mc.world.playerEntities) {
            if (player != null && !EntityUtil.isDead((Entity)player) && EntityUtil.getHealth((Entity)player) <= this.health.getValue() && !player.equals((Object)BowSpam.mc.player) && !Thunderhack.friendManager.isFriend(player) && BowSpam.mc.player.getDistanceSq((Entity)player) <= MathUtil.square(this.range.getValue())) {
